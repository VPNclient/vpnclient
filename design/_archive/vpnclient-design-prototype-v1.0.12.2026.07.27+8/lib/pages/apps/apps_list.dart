...
class AppsListState extends State<AppsList> {
  ...
  // TODO(real-installed-apps): per Figma's Apps/Readme2 note, the real app
  // must list actually-installed apps sorted by install date (newest first).
  // This mock hardcodes a fixed 4-app list instead. To do this for real:
  // query installed packages via a platform channel (Android: PackageManager
  // + PackageInfo.firstInstallTime through e.g. the `device_apps` or a custom
  // MethodChannel; iOS can't enumerate installed apps at all, so this screen
  // is Android-only in practice), map each to {icon/image, text, isActive},
  // sort by install timestamp descending, then feed the result in here
  // instead of the static `appsList` below.
